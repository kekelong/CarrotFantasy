...
namespace Carrot
{
    ...
    public class UIFacade
    {
        ...
        //UI面板
        public Dictionary<string, IPanel> currentWindowPanelDict = new Dictionary<string, IPanel>();
        ...
        //场景状态
        public IWindowState currentWindowState;
        public IWindowState lastWindowState;

        public UIFacade(UIWindowManager windowManager)
        {
        ...
        public void InitMask()
        {
            ...
            mask = CreateUIAndSetUIPosition("Mask");
            maskImage = mask.GetComponent<Image>();
        }


        //改变当前场景的状态
        public void ChangeSceneState(IWindowState windowState)
        {
        ...
        {
            mask.transform.gameObject.SetActive(true);
        ...
        {
            lastWindowState.ExitWindow();
            currentWindowState.EnterWindow();
            HideMask();
        }
        ...
        public void HideMask()
        {
            ...
            mask.transform.gameObject.SetActive(false);
        }
        ...
        {
            foreach (var item in windowManager.currentWindowPanelDict)
            {
                item.Value.transform.SetParent(canvasTransform);
                item.Value.transform.localPosition = Vector3.zero;
                item.Value.transform.localScale = Vector3.one;
                IPanel basePanel = item.Value.GetComponent<IPanel>();
                if (basePanel == null)
                {
                    Debug.Log("获取面板上IPanel脚本失败");
                }
                basePanel.InitPanel();
                currentWindowPanelDict.Add(item.Key, basePanel);
            }
        }
        ...
        {
            windowManager.currentWindowPanelDict.Add(uiPanelName, GetGameObjectResource(FactoryType.UIPanelFactory, uiPanelName));
        }
